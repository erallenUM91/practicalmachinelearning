...
The training data was partitioned into a training and test data sets for cross-validation purposes.  

```{r}
# create training partitions for cross-validation
...
The model was build using Random Forest with the remaining fields and default settings. 
...
set.seed(3825)
# modRF <- train(classe ~., method="rf",data=dftrn)
confm <- confusionMatrix(dft$classe,predict(modRF,newdata = dft))

```

Cross-validation against the training partition (out of sample) shows and Accuracy of `r confm$overall[[1]]` with a confidence interval from `r confm$overall[[3]]` to `r confm$overall[[4]]`.
